...
<div data-base-accordion-immediate>…</div>`,
...
function bindDialogOpenButton(appRoot: HTMLElement, appInstance: App) {
	...
	const openDialog = (frame = 0) => {
		const api = extractComponent(host, BaseDialog, appInstance) as { open: () => void } | undefined;
		if (api) {
			api.open();
			return;
		}
